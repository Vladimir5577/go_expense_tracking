...
func spaHandler(webDir string) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Имена файлов статики не содержат хеша: app.css и модули меняются
		// по месту, и без явного заголовка браузер вправе держать их в кеше
		// без проверки — после деплоя пользователь получит старый CSS.
		// no-cache означает не «не кешировать», а «спроси перед использованием»:
		// если файл не менялся, ответом будет пустой 304.
